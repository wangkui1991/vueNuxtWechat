...
export default {
  ...
  computed: {
    ...mapState({
      ...
      product: state => state.product.focusProduct
    })
  },
  methods: {
    ...
    async handPayment () {
      ...
      if (!name || !address || !phoneNumber) {
        toggleModal(this.modal, '收货信息忘填了喔')
        return
      }
      const res = await this.$store.dispatch('product/createOrder', {
  ...
  async beforeMount () {
    ...
    this.$store.dispatch('product/focusProduct', id)
